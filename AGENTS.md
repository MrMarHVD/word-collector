...
- EVERY TIME you make a change to the code (frontend, backend, CSS, templates, anything), immediately restart the relevant dev server(s) so the change is actually being served. Do not assume `--watch` or any other mechanism is picking it up — kill the existing process on the relevant port and start a fresh one from the correct worktree directory. CSS changes additionally require rebuilding Tailwind (`npm run build:css` from the worktree root, or run the watcher).
