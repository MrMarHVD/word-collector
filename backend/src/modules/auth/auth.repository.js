export function createAuthRepository(db) {
  const userByEmail = db.prepare("SELECT id, email, native_language AS nativeLanguage, password_hash AS passwordHash, password_salt AS passwordSalt FROM users WHERE lower(email) = lower(?)");
  const userById = db.prepare("SELECT id, email, native_language AS nativeLanguage FROM users WHERE id = ?");
