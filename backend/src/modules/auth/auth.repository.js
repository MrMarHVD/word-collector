export function createAuthRepository(db) {
  const userByEmail = db.prepare(`SELECT id, email, native_language AS "nativeLanguage", practice_words_per_session AS "practiceWordsPerSession", email_verified AS "emailVerified", (password_hash IS NOT NULL AND password_salt IS NOT NULL) AS "hasPassword", password_hash AS "passwordHash", password_salt AS "passwordSalt" FROM users WHERE lower(email) = lower(?)`);
  const userById = db.prepare(`SELECT id, email, native_language AS "nativeLanguage", practice_words_per_session AS "practiceWordsPerSession", email_verified AS "emailVerified", (password_hash IS NOT NULL AND password_salt IS NOT NULL) AS "hasPassword" FROM users WHERE id = ?`);
  ...
  const oauthAccountByProviderUser = db.prepare(`SELECT user_id AS "userId", provider, provider_user_id AS "providerUserId", email FROM oauth_accounts WHERE provider = ? AND provider_user_id = ?`);
  const createOAuthAccount = db.prepare("INSERT INTO oauth_accounts (user_id, provider, provider_user_id, email) VALUES (?, ?, ?, ?) ON CONFLICT (provider, provider_user_id) DO UPDATE SET email = EXCLUDED.email, updated_at = now()");
  ...
  const sessionUser = db.prepare(`SELECT u.id, u.email, u.native_language AS "nativeLanguage", u.email_verified AS "emailVerified", (u.password_hash IS NOT NULL AND u.password_salt IS NOT NULL) AS "hasPassword" FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.id = ? AND s.expires_at > now()`);
  ...
  return {
    ...
    createOAuthAccount(userId, provider, providerUserId, email) {
      return createOAuthAccount.run(userId, provider, providerUserId, email);
    },
